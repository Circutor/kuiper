...
.PHONY: arm
arm:
	GO111MODULE=on GOOS=linux GOARCH=arm CGO_ENABLED=0 go build -ldflags="-s -w -X main.Version="1.0.0-cir" -X main.LoadFileType=relative" -o kuiper  xstream/cli/main.go;
	GO111MODULE=on GOOS=linux GOARCH=arm CGO_ENABLED=0 go build -ldflags="-s -w -X main.Version="1.0.0-cir" -X main.LoadFileType=relative" -o kuiperd xstream/server/main.go;
